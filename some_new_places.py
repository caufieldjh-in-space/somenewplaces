...
with open("dai.keys") as dai_api_file:
	dai_api_keys = []
	for line in dai_api_file:
		dai_api_keys.append(line.rstrip())
...
def get_gen_text(text):
	'''Gets text generated by a GPT-2 model through an API hosted
	by DeepAI.org. Function returns one sentence and no more than 
	100 characters.'''
	...
	print("Retrieving text from GPT-2 on DeepAI...")
	
	req = requests.post("https://api.deepai.org/api/text-generator",
						data={"text":text},
